...
$(document).ready(function() {
    ...
    $("#listalldrop").change(function() {
        ...
        //alert(selected + "has" + JSON.stringify(stuff[selected]));
        var linkLang = "in";
        if (selected != 0) {
            $("#listresultdiv").empty();
            var cleanData = removeJunk(stuff[selected - 1]);
            linkLang = properList(cleanData);
            var link = "http://"+ linkLang +".yahoo.com/";
            $("#listresultdiv").append("<a href='" + link + "' target='_blank'><button>Visit</button></a>");
        }
        else{
            $("#listresultdiv").empty();
        }
...
function properList(cleanData){
    for(var i=0;i<primeObjFields.length;i++){
        var key = primeObjFields[i].toUpperCase();
        $("#listresultdiv").append("<p><h5 style='display:inline;'>" + key + ":</h5>  " + cleanData[primeObjFields[i]] + "</p>");
    }
...
function dropdown(listData) {
    //alert(JSON.stringify(listData.resultSet));
